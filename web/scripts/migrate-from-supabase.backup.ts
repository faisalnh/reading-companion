...
async function migrateTable(
  ...
  for (const row of data) {
    try {
      ...
      if (error.code === "23505") {
        ...
        // Foreign key violation - log but continue
        console.error(
          `  ⚠️  Foreign key error (row will be skipped):`,
          error.detail,
        );
        skipped++;
      } else {
  ...
  console.log(
    `  ✓ Inserted ${inserted} rows, skipped ${skipped}, errors ${errors}`,
  );
}
  ...
  for (const column of columnsToMap) {
    if (mappedRow[column]) {
      ...
      if (mapping) {
        ...
        console.warn(
          `    ⚠️  No mapping found for ${column}: ${mappedRow[column]}`,
        );
