...
export const RoleChecker = () => {
  ...
  const isAuthorized = roleInfo?.profile?.role === 'LIBRARIAN' || roleInfo?.profile?.role === 'ADMIN';
  ...
  return (
    ...
    >
      <h3 className="mb-2 text-sm font-semibold text-white">Your Account Status</h3>
      <div className="space-y-1 text-sm">
        <p className="text-slate-300">
          <span className="font-medium">Email:</span> {roleInfo?.email}
        </p>
        <p className="text-slate-300">
          <span className="font-medium">User ID:</span> {roleInfo?.userId?.substring(0, 8)}...
        </p>
        {roleInfo?.profileError ? (
          <p className="text-red-400">
            <span className="font-medium">Profile Error:</span> {roleInfo.profileError}
          </p>
        ) : (
          <>
            <p className="text-slate-300">
            ...
            {roleInfo?.profile?.full_name && (
              <p className="text-slate-300">
                <span className="font-medium">Name:</span> {roleInfo.profile.full_name}
              </p>
            )}
            <p className="text-slate-300">
              <span className="font-medium">Profiles Found:</span>{' '}
              <span className={roleInfo?.hasDuplicates ? 'text-red-400 font-bold' : ''}>
...
SET role = 'ADMIN';`}
      ...
      {roleInfo?.hasDuplicates && (
        <div className="mt-3 rounded border border-red-600 bg-red-900/30 p-3">
          <p className="text-xs font-semibold text-red-300">🚨 Critical: Duplicate Profiles</p>
          <p className="mt-1 text-xs text-red-200">
            You have {roleInfo.profileCount} duplicate profile records. This must be fixed first!
          </p>
          <p className="mt-2 text-xs text-red-200">
...
WHERE id = '${roleInfo?.userId}';`}
      ...
      {!isAuthorized && !roleInfo?.hasDuplicates && !noProfile && (
        <div className="mt-3 rounded border border-yellow-600 bg-yellow-900/30 p-3">
          <p className="text-xs font-semibold text-yellow-300">⚠️ Action Required</p>
          <p className="mt-1 text-xs text-yellow-200">
            Your role is currently <strong>{roleInfo?.profile?.role || 'UNKNOWN'}</strong>. Only
