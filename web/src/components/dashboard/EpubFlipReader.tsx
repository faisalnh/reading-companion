...
export const EpubFlipReader = forwardRef<EpubFlipReaderRef, EpubFlipReaderProps>(
    ({ epubUrl, initialPage = 1, onPageChange, bookTitle }, ref) => {
        ...
        const basePage = typeof initialPage === 'string' ? parseInt(initialPage, 10) : (initialPage || 0);
        ...
        if (savedPageRef.current === null && typeof window !== 'undefined') {
            try {
                ...
                const dbValue = typeof initialPage === 'string' ? parseInt(initialPage, 10) : initialPage;
                ...
                savedPageRef.current = typeof initialPage === 'string' ? parseInt(initialPage, 10) : initialPage;
            }
        }
        ...
        useEffect(() => {
            const parseEpub = async () => {
                ...
                try {
                        ...
                        while (remainingText.length > 0) {
                            ...
                            if (remainingText.length > CHARS_PER_PAGE) {
                                ...
                                if (paragraphBreak > CHARS_PER_PAGE * 0.5) {
                                    ...
                                    // Try sentence break
                                    const sentenceBreak = remainingText.lastIndexOf(". ", CHARS_PER_PAGE);
                                    if (sentenceBreak > CHARS_PER_PAGE * 0.5) {
                                        breakPoint = sentenceBreak + 2;
                                    } else {
                                        // Try word break
                                        const wordBreak = remainingText.lastIndexOf(" ", CHARS_PER_PAGE);
                    ...
                    if (finalPages.length <= 2) { // Only covers
        ...
        const getResponsiveDimensions = () => {
            ...
            if (isFullscreen) {
                ...
                // Classic book ratio approx 2:3 per page
                const pageWidth = (availableHeight * 0.66);
                // Ensure two pages fit
                if (pageWidth * 2 > availableWidth) {
                    return { baseWidth: (availableWidth / 2) - 10, baseHeight: ((availableWidth / 2) - 10) * 1.5 };
                }
                return { baseWidth: pageWidth, baseHeight: availableHeight };
            }
        ...
        useEffect(() => {
            if (bookRef.current && pages.length > 0) {
                ...
                const targetPage = lastPageRef.current || savedPageRef.current || initialPage;
                const targetIndex = Math.round(targetPage);

                console.log('🎯 Attempting navigation to:', { targetPage, targetIndex, pagesCount: pages.length });
        ...
        return (
            <div
                ref={containerRef}
                className={clsx(
                    "book-container", // New container class for 3D perspective
                    isFullscreen
                        ? "fixed inset-0 z-[9999] overflow-hidden flex items-center justify-center"
                        : "space-y-4 py-8",
                    themeClasses
                )}
                style={isFullscreen ? { backgroundColor: "#1a1a1a" } : {}}
            >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-purple-200 bg-gradient-to-r from-purple-50 to-pink-50 p-2 shadow-sm">
                        ...
                        <div className="flex items-center gap-2">
                            <div className="text-xs font-medium text-gray-500 mr-2">
                                {currentPage === 0 ? "Cover" : `Pages ${currentPage}-${currentPage + 1}`}
                            </div>
                            <button
                                onClick={toggleFullscreen}
                ...
                <div
                    className={clsx(
                        ...
                        isFullscreen ? "flex-1 flex items-center justify-center p-0" : "p-4"
                    )}
                >
                    <HTMLFlipBook
                        key={`epub-${preferences.fontSize}-${preferences.theme}`}
                        ref={bookRef}
                        width={width}
                        height={height}
                            ...
                            if (page.type === "back-cover") {
                                return (
                                    <div key={`back-${index}`} className="book-cover">
                                        <div className="text-white/50 text-sm mt-auto mb-8">End of Book</div>
                                    </div>
                                );
                            }
                    ...
                    <div className="rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 px-4 py-3">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <p className="text-sm text-indigo-600">
                                ...
                                {totalPages - 2} {/* Subtract cover and back cover */}
                            </p>
                            <div className="flex items-center gap-2">
                                <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                                <p className="text-xs font-semibold text-emerald-700">EPUB Mode</p>
                            </div>
                        </div>
                        <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-indigo-100">
                            <div
                                className="h-full bg-gradient-to-r from-emerald-500 to-teal-500 transition-all duration-300"
                                style={{ width: `${Math.max(0, (currentPage / Math.max(1, totalPages - 2))) * 100}%` }}
                            ></div>
                        </div>
                    </div>
                )}
