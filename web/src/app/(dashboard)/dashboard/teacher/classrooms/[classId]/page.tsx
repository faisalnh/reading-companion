...
export default async function ManageClassroomPage({ params }: { params: Promise<{ classId: string }> }) {
  ...
  const classStudents =
    rosterData?.map((entry) => ({
      id: entry.student_id,
      full_name: entry.profiles?.full_name ?? 'Unknown student',
    })) ?? [];
  ...
  if (rosterStudentIds.length > 0) {
    ...
    readings = (readingsData ?? []) as typeof readings;
  }
  ...
  if (rosterStudentIds.length > 0) {
    const { data: quizData, error: quizError } = await supabaseAdmin
      .from('quiz_attempts')
      .select('score, submitted_at, profiles!quiz_attempts_student_id_fkey(full_name), quizzes(id, books(title))')
      .in('student_id', rosterStudentIds)
    ...
    quizAttempts = (quizData ?? []) as typeof quizAttempts;
  }

  const { data: studentDirectory, error: studentDirectoryError } = await supabaseAdmin
    .from('profiles')
    .select('id, full_name')
    .eq('role', 'STUDENT');
  ...
  const availableStudents =
    studentDirectory
      ?.filter((student) => rosterIdSet.has(student.id) || !assignedIds.has(student.id))
      .map((student) => ({
        id: student.id,
        full_name: student.full_name ?? '',
      })) ?? [];

  const { data: assignedBookRows, error: assignedBooksError } = await supabaseAdmin
    .from('class_books')
  ...
  const assignedBooks =
    assignedBookRows
      ?.map((entry) => ({
        ...
        title: entry.books?.title ?? 'Untitled',
        author: entry.books?.author ?? null,
        cover_url: entry.books?.cover_url ?? null,
  ...
  const availableBooks =
    allBooksData
      ?.filter((book) => !assignedBooks.some((assigned) => assigned.book_id === book.id))
      .map((book) => ({
        id: book.id,
        title: book.title,
    ...
    <div className="space-y-8">
      <section className="rounded-[32px] border border-white/70 bg-white/95 p-6 shadow-[0_25px_70px_rgba(147,118,255,0.2)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-indigo-400">Manage Classroom</p>
            <h1 className="text-3xl font-black text-indigo-950">{classroom.name}</h1>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-[28px] border border-white/70 bg-white/90 p-6 shadow-[0_20px_60px_rgba(79,70,229,0.18)]">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-rose-400">Student Progress</p>
            <h2 className="text-xl font-black text-indigo-950">Reading updates</h2>
            <p className="text-sm text-indigo-500">Latest progress from this classroom only.</p>
          </div>
          <div className={tableWrapperClass}>
            <table className={tableClass}>
              ...
              <tbody className="divide-y divide-indigo-50">
                {readings.length > 0 ? (
                  readings.map((entry) => (
                    <tr key={`${entry.student_id}-${entry.books?.title ?? 'book'}`} className="hover:bg-indigo-50/60">
                      ...
                      <td className={cellClass}>{formatDate(entry.completed_at ?? entry.started_at)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
        ...
        <div className="space-y-4 rounded-[28px] border border-white/70 bg-white/90 p-6 shadow-[0_20px_60px_rgba(16,185,129,0.18)]">
          <div>
            ...
            <h2 className="text-xl font-black text-indigo-950">Recent quiz attempts</h2>
            <p className="text-sm text-indigo-500">See how this class is performing.</p>
          </div>
          <div className={tableWrapperClass}>
            <table className={tableClass}>
              <thead className={headClass}>
      ...
      <section className="rounded-[32px] border border-white/70 bg-white/95 p-6 shadow-[0_25px_70px_rgba(255,173,109,0.2)]">
        <ClassroomRoster classId={classId} students={classStudents} allStudents={availableStudents} />
      </section>

      <ClassReadingList classId={classId} assignedBooks={assignedBooks} availableBooks={availableBooks} />
    </div>
  );
}
