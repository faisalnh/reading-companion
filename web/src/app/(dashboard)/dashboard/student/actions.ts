...
export const recordReadingProgress = async (input: { bookId: number; currentPage: number }) => {
...
export const evaluateAchievements = async () => {
  ...
  const finishedCount =
    completions?.filter((entry) => {
      const pageCount = (entry.books as { page_count?: number } | null)?.page_count ?? 0;
  ...
  const earnedIds = new Set((earned ?? []).map((record) => record.achievement_id));
  ...
  const toAward =
    achievements?.filter((achievement) => {
      ...
      const criteria = (achievement.criteria as { type?: string; count?: number } | null) ?? {};
      if (criteria.type === 'books_read' && typeof criteria.count === 'number') {
  ...
  await supabase
    .from('student_achievements')
    .insert(toAward.map((achievement) => ({ student_id: user.id, achievement_id: achievement.id })));
