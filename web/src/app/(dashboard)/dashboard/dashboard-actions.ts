 ...
 */
export async function getTeacherOverview(userId: string, profileId: string, role: string) {
  try {
    // Get teacher's classes (or all classes for admin)
    const classesQuery = role === "TEACHER"
      ? `SELECT id, name FROM classes WHERE teacher_id = $1`
      : `SELECT id, name FROM classes`;

    const classesParams = role === "TEACHER" ? [profileId] : [];
    const classesResult = await queryWithContext(userId, classesQuery, classesParams);
    ...
    const totalAssignments = parseInt(totalAssignmentsResult.rows[0]?.count || "0");
    ...
    const completedAssignments = parseInt(completedAssignmentsResult.rows[0]?.count || "0");
    ...
    return {
      ...
      data: {
        ...
        completionRate,
        activeAssignments,
        recentCompletions: recentCompletionsResult.rows.map((entry) => ({
