...
export default async function LibrarianPage() {
  ...
  const { data: bookRows } = await supabase
    .from('books')
    .select(
      'id, isbn, title, author, publisher, publication_year, genre, language, description, page_count, pdf_url, cover_url, created_at, book_access(access_level)',
    )
  ...
  const managedBooks: ManagedBookRecord[] =
    bookRows?.map((book) => ({
      ...
      publicationYear: book.publication_year ?? (book.created_at ? new Date(book.created_at).getFullYear() : CURRENT_YEAR),
      genre: book.genre ?? '',
      ...
      accessLevels:
        book.book_access?.map((entry: { access_level: AccessLevelValue }) => entry.access_level as AccessLevelValue) ?? [],
    })) ?? [];

  const genreOptions = Array.from(
    new Set(managedBooks.map((book) => book.genre).filter((value) => Boolean(value && value.trim()))),
  ).sort((a, b) => a.localeCompare(b));

  const languageOptions = Array.from(
    new Set(managedBooks.map((book) => book.language).filter((value) => Boolean(value && value.trim()))),
  ).sort((a, b) => a.localeCompare(b));

  return (
    <div className="space-y-8">
      <BookManagementSection books={managedBooks} genreOptions={genreOptions} languageOptions={languageOptions} />
      <QuizGenerator
