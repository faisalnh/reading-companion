...
export default function LibrarianNav({ user }: LibrarianNavProps) {
  const pathname = usePathname();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const navItems = [
    { name: 'Dashboard', href: '/dashboard/librarian', icon: '📊' },
    { name: 'Book Collection', href: '/dashboard/librarian/books', icon: '📚' },
    { name: 'Upload Books', href: '/dashboard/librarian/upload', icon: '📤' },
    { name: 'Categories', href: '/dashboard/librarian/categories', icon: '🏷️' },
  ];
          ...
          <div className="p-6 border-b">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800 truncate">{user.name || 'Librarian'}</p>
                <p className="text-sm text-gray-500 truncate">{user.email}</p>
                <span className="inline-block mt-1 px-2 py-0.5 bg-accent-100 text-accent-700 text-xs font-semibold rounded-full">
                  Librarian
                </span>
              </div>
            </div>
